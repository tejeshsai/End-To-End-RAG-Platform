...
class PdfReader:
    ...
    def get_pages(self, page_number_start: int, page_number_end: int) -> list[str]:
        ...
        if (
            page_number_start < 0
            or page_number_end < 0
            or page_number_start >= len(self.pages)
            or page_number_end >= len(self.pages)
        ):
            raise IndexError(
                f"Page number {page_number_start} or {page_number_end} is out of range"
            )
        if page_number_start > page_number_end:
            raise ValueError(
                f"Page number {page_number_start} is greater than {page_number_end}"
            )
        if page_number_start == page_number_end:
    ...
    def save_json(self, json_path: str):
        json_data = {"length": self.get_page_count(), "pages": []}
        ...
        for page_number, page in enumerate(self.pages):
            json_data["pages"].append(
                {"page_number": page_number, "page_text": page.strip()}
            )
