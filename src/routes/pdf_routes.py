    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            vector_store.add_documents(
                doc_id=f"{i}", text=chunk, embedding=embedding)

    except Exception as e:
        raise HTTPException(
            status_code=400, detail=f"Error processing the file : {str(e)}")
...
async def semantic_search(q: str = (Query(..., description="Search Query"))):
    try:
        ...
        vector_store_results = vector_store.query(
        ...
        raise HTTPException(
            status_code=400, detail=f"Error processing the query : {str(e)}")
